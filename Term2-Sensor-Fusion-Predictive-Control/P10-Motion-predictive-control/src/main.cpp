      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          // Predict (x = y = psi = 0)
          double predicted_x    = 0.0 + v * dt;
          double predicted_y    = 0.0;
          double predicted_psi  = 0.0 - v * delta / Lf * dt;
          ...
          next_x_vals.resize(ptsx_transform.size());
          next_y_vals.resize(ptsy_transform.size());

          for (int i = 0; i < ptsx_transform.size(); i++) {
            next_x_vals[i] = ptsx_transform[i];
            next_y_vals[i] = ptsy_transform[i];
          }
