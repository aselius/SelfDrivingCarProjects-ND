...
const int current_delta_weight = 1;
const int diff_delta_weight    = 200;
const int current_a_weight     = 1;
const int diff_a_weight        = 1;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N - 1; t++) {
      fg[0] += current_delta_weight * CppAD::pow(vars[delta_start + t], 2);
      fg[0] += current_a_weight * CppAD::pow(vars[a_start + t], 2);
    }
